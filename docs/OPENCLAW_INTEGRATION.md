...
**CLI Command**: `openopenleash task start --name "{{name}}" --ttl {{ttl}} --base-path /tmp/openleash-tasks`
...
**CLI Command**: `openopenleash request install --manager pip --package "{{package}}" --task_id {{task_id}} --reason "{{rationale}}"`
...
**CLI Command**: `openopenleash run --task-id {{task_id}}`
...
3.  **Agent**: *Invokes Tool* → `openopenleash task start --name "Web Scrape"`
4.  **Agent**: *Invokes Tool* → `openopenleash request install --package pandas --task-id <ID> --reason "CSV processing"`
