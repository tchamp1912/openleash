...
OpenLeash provides a secure "Capability Broker" for AI agents. This skill allows you to interact with system resources (Keychain, Pip, NPM, Brew, Shell) without exposing sensitive credentials to your context window.
...
```bash
openleash task start --name "Mission Name" --ttl 3600
```
...
```bash
openleash request install --manager pip --package <name> --task_id <TASK_ID> --reason "<why>"
```

### 3. Secure Execution with Secret Injection
**CRITICAL**: Never read secrets into your own context window (e.g., using `openopenleash request secret`). Instead, use `openopenleash exec` to inject them directly into the environment of the tool you are running.

```bash
# Injects the secret 'anthropic/api-key' into the env var 'API_KEY' for your script
openleash exec --task_id <TASK_ID> --secret API_KEY=anthropic/api-key -- python3 scrape.py
```
...
The `openopenleash run` command exports the `PATH` and `VIRTUAL_ENV` environment variables that you can source to execute commands directly in your sandbox.
...
- **In-Memory Secrets Only**: Always use `openopenleash exec` for credentials.
- **Provide clear rationales**: Every `reason` field is logged in a hash-chained audit ledger.
- **Fail-Secure**: If a command is denied, request human approval or adjust `~/.openleash/policies.yaml`.
