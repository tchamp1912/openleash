...
use uuid::Uuid;
use leash_ai_core::models::{TaskStatus, ResourceType};
use leash_ai_backend::SecretBackend;
use crate::LeashDaemon;

pub async fn run_background_worker(daemon: Arc<LeashDaemon>) {
    loop {
...
pub async fn run_telegram_bot(token: String, daemon: Arc<LeashDaemon>) {
    let bot = Bot::new(token);

    let handler = Update::filter_callback_query().endpoint(
        |bot: Bot, q: CallbackQuery, daemon: Arc<LeashDaemon>| async move {
                        ...
                        if status == "Approved" {
                            if let Ok(Some(req)) = daemon.db.get_approval_by_id(&approval_id).await {
                                if req.resource_type == ResourceType::System && req.resource_id == "keychain-unlock" {
                                    let password = std::env::var("LEASH_KEYCHAIN_PASSWORD").ok();
