...
use leash_ai_api::pb::{ListPendingApprovalsRequest, ListPendingApprovalsResponse, ApproveRequest, ApproveResponse, DenyRequest, DenyResponse, PendingApproval};
use crate::LeashDaemon;
use uuid::Uuid;

#[tonic::async_trait]
impl ApprovalService for LeashDaemon {
    ...
    async fn approve(
        ...
        let override_scope = req.scope.and_then(|s| match s.to_lowercase().as_str() {
            "once" => Some(leash_ai_core::models::ApprovalScope::Once),
            "task" => Some(leash_ai_core::models::ApprovalScope::Task),
            "permanent" => Some(leash_ai_core::models::ApprovalScope::Permanent),
            _ => None,
        });
