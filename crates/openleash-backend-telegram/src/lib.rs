...
use openleash_core::models::ApprovalRequest;
...
impl ApprovalBackend for TelegramApprovalBackend {
    async fn notify_approval(&self, req: &ApprovalRequest) -> Result<()> {
        ...
        let (icon, title) = match req.resource_type {
            openleash_core::models::ResourceType::Secret => ("🔑", "Secret Request"),
            openleash_core::models::ResourceType::Package => ("📦", "Package Request"),
            openleash_core::models::ResourceType::System => {
...
<b>Expires</b>: {}",
        ...
        bot.send_message(ChatId(self.chat_id), message)
            ...
            .await
            .map_err(|e| OpenLeashError::Backend(format!("Telegram error: {}", e)))?;
