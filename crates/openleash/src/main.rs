...
use openleash_client::client::OpenLeashClient;
use anyhow::Context;
...
#[derive(Parser)]
#[command(name = "openleash")]
#[command(about = "OpenLeash CLI", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,

    #[arg(short, long, default_value = "unix:///tmp/openleash.sock")]
    server: String,
}

#[derive(Subcommand)]
enum Commands {
    /// Initialize OpenLeash environment
    Init {
        #[arg(short, long)]
        force: bool,
    },
    /// Install OpenLeash as a system service (macOS LaunchAgent or Linux Systemd)
    Install {
        #[arg(short, long)]
        user: bool,
    },
...
async fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        Commands::Init { force } => {
            ...
            let openleash_dir = std::path::PathBuf::from(format!("{}/.openleash", home));
            
            if openleash_dir.exists() && !*force {
                println!("OpenLeash is already initialized at {:?}. Use --force to re-initialize.", openleash_dir);
                return Ok(());
            }

            std::fs::create_dir_all(&openleash_dir)?;
            let profiles_dir = openleash_dir.join("profiles");
            std::fs::create_dir_all(&profiles_dir)?;

            // 1. Interactive Telegram Setup
            let mut config = openleash_core::config::OpenLeashConfig::default();
            ...
            if enable_tg.trim().to_lowercase() == "y" {
                ...
                if let Ok(chat_id) = chat_id_str.trim().parse::<i64>() {
                    ...
                    config.telegram = Some(openleash_core::config::TelegramConfig {
            ...
            if config.backends.telegram.enabled {
                policies.push(openleash_core::models::Policy {
                    ...
                    resource_type: openleash_core::models::ResourceType::Secret,
                    priority: 20,
            ...
            let policies_path = openleash_dir.join("policies.yaml");
            ...
            use openleash_core::sandbox::{SandboxProfileGenerator, SandboxLevel, NetworkMode};
            ...
            std::fs::copy(profiles_dir.join("permissive-open.sb"), openleash_dir.join("agent.sb"))?;
            println!("Created sandbox profiles in {:?}/profiles/", openleash_dir);
            println!("Default profile linked to {:?}", openleash_dir.join("agent.sb"));

            println!("\nInitialization complete! Next steps:");
            println!("1. Install the background service: openleash install");
            println!("2. Start the daemon (macOS): launchctl load ~/Library/LaunchAgents/io.openleash.openleashd.plist");
            println!("3. Run your agent: sandbox-exec -f {:?}/agent.sb <your-agent-command>", openleash_dir);
        ...
        Commands::Install { user: _user } => {
            ...
            // Try to find openleashd executable
            let current_exe = std::env::current_exe()?;
            let bin_dir = current_exe.parent().context("Failed to get bin dir")?;
            let openleashd_path = bin_dir.join("openleashd");
            
            if !openleashd_path.exists() {
                anyhow::bail!("openleashd executable not found at {:?}. Please ensure it is built and in the same directory as leash.", openleashd_path);
            }

            match std::env::consts::OS {
                "macos" => {
                    println!("Installing OpenLeash as a macOS LaunchAgent...");
                    let plist_content = format!(r#"<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>io.openleash.openleashd</string>
    ...
    <string>{}/openleashd.out.log</string>
    <key>StandardErrorPath</key>
    <string>{}/openleashd.err.log</string>
</dict>
</plist>
"#, openleashd_path.to_string_lossy(), config_path.to_string_lossy(), openleash_dir.to_string_lossy(), openleash_dir.to_string_lossy());

                    let agents_dir = std::path::PathBuf::from(format!("{}/Library/LaunchAgents", home));
                    std::fs::create_dir_all(&agents_dir)?;
                    let plist_path = agents_dir.join("io.openleash.openleashd.plist");
                ...
                "linux" => {
                    println!("Installing OpenLeash as a systemd user service...");
                    let service_content = format!(r#"[Unit]
Description=OpenLeash Daemon
...
Restart=always
RestartSec=5
StandardOutput=file:{}/openleashd.out.log
StandardError=file:{}/openleashd.err.log
...
"#, openleashd_path.to_string_lossy(), config_path.to_string_lossy(), openleash_dir.to_string_lossy(), openleash_dir.to_string_lossy());
                    ...
                    let service_path = systemd_dir.join("openleashd.service");
                    std::fs::write(&service_path, service_content)?;

                    println!("Created systemd service: {:?}", service_path);
                    println!("\nTo start the service now:");
                    println!("  systemctl --user enable --now openleashd");
                    println!("\nTo check status:");
                    println!("  systemctl --user status openleashd");
                },
                _ => {
        ...
        Commands::Sandbox { operation } => match operation {
            ...
            SandboxCommands::Generate { profile, output } => {
                ...
                let home = std::env::var("HOME")?;
                let current_dir = std::env::current_dir()?.to_string_lossy().to_string();
                let config = openleash_core::config::OpenLeashConfig::load(None).unwrap_or_default();
        ...
        Commands::Request { resource } => match resource {
            RequestCommands::Install { manager, package, scope, reason, ttl, task_id } => {
                let mut client = OpenLeashClient::connect(cli.server).await
