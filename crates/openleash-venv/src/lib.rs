...
use openleash_core::config::NodeBootstrapConfig;
...
impl VenvManager {
    ...
    pub async fn ensure_created(&self) -> Result<()> {
        if !self.root_path.exists() {
            ...
            let status = Command::new("python3")
                ...
                .status()
                .await
                .map_err(|e| OpenLeashError::Backend(format!("Failed to create venv: {}", e)))?;

            if !status.success() {
                return Err(OpenLeashError::Backend(format!("python3 -m venv failed with status {}", status)));
            }
        }
        Ok(())
    }
    ...
    pub async fn ensure_brew_bootstrapped(&self) -> Result<()> {
        ...
        if !brew_dir.exists() {
            ...
            tokio::fs::create_dir_all(&brew_dir).await
                .map_err(|e| OpenLeashError::Backend(format!("Failed to create brew dir: {}", e)))?;
            ...
            let status = Command::new("git")
                ...
                .status()
                .await
                .map_err(|e| OpenLeashError::Backend(format!("Failed to clone Homebrew: {}", e)))?;

            if !status.success() {
                return Err(OpenLeashError::Backend(format!("Homebrew clone failed with status {}", status)));
            }
        }
        Ok(())
    }
    ...
    pub async fn ensure_node_bootstrapped(&self, config: &NodeBootstrapConfig) -> Result<()> {
        ...
        tokio::fs::create_dir_all(&node_dir).await
            .map_err(|e| OpenLeashError::Backend(format!("Failed to create node dir: {}", e)))?;
        ...
        let status = Command::new("curl")
            ...
            .status()
            .await
            .map_err(|e| OpenLeashError::Backend(format!("Failed to run curl: {}", e)))?;
        if !status.success() {
            let _ = tokio::fs::remove_file(&archive_path).await;
            return Err(OpenLeashError::Backend("Failed to download Node.js tarball".to_string()));
        }
        ...
        let status = Command::new("tar")
            ...
            .status()
            .await
            .map_err(|e| OpenLeashError::Backend(format!("Failed to run tar: {}", e)))?;

        let _ = tokio::fs::remove_file(&archive_path).await;
        if !status.success() {
            return Err(OpenLeashError::Backend("Failed to extract Node.js tarball".to_string()));
        }

        if !npm_bin.exists() {
            return Err(OpenLeashError::Backend(format!(
    ...
    pub async fn remove(&self) -> Result<()> {
        if self.root_path.exists() {
            tokio::fs::remove_dir_all(&self.root_path)
                .await
                .map_err(|e| OpenLeashError::Backend(format!("Failed to remove venv: {}", e)))?;
        }
        Ok(())
    }
}
...
fn platform_for_node() -> Result<(String, String)> {
    let os = match std::env::consts::OS {
        ...
        "linux" => "linux",
        "windows" => {
            return Err(OpenLeashError::Backend(
