...
impl SandboxProfileGenerator {
    ...
    pub fn with_config(mut self, config: &LeashConfig) -> Self {
    ...
    pub fn generate(&self) -> String {
        let mut sb = String::from(";; Leash AI Agent Sandbox Profile
(version 1)
(deny default)
...
"#, self.project_dir));
                sb.push_str(&format!(r#"(allow file-read* (subpath "{}/.leash"))
"#, self.home_dir));
            }
        }
...
;; Leash Infrastructure
");
        sb.push_str(r#"(allow file-read* file-write* (literal "/tmp/leash.sock"))
"#);
        sb.push_str(r#"(allow file-read* file-write* (subpath "/tmp/leash-tasks"))
"#);
        sb.push_str(r#"(allow process-exec (subpath "/tmp/leash-tasks"))
"#);
