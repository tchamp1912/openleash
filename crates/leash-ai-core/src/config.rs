...
impl Default for LeashConfig {
    fn default() -> Self {
        ...
        let config_dir = format!("{}/.leash", home);
        
        Self {
            server: ServerConfig {
                uds_path: "/tmp/leash.sock".to_string(),
...
impl LeashConfig {
    ...
    pub fn load(path: Option<PathBuf>) -> Result<Self, crate::error::LeashError> {
        let path = path.unwrap_or_else(|| {
            let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
            PathBuf::from(format!("{}/.leash/config.yaml", home))
        });
        ...
        let content = std::fs::read_to_string(path)
            .map_err(|e| crate::error::LeashError::Config(format!("Failed to read config file: {}", e)))?;
        
        serde_yaml::from_str(&content)
            .map_err(|e| crate::error::LeashError::Config(format!("Failed to parse config YAML: {}", e)))
    }
}
