...
use openleash_core::{models::{Lease, LeaseStatus, AuditEvent, ResourceType, PackageManager}, Result, OpenLeashError};
use uuid::Uuid;
...
impl Db {
    ...
    pub async fn insert_approval_request(&self, req: &openleash_core::models::ApprovalRequest) -> Result<()> {
        sqlx::query(
    ...
    pub async fn get_pending_approvals(&self) -> Result<Vec<openleash_core::models::ApprovalRequest>> {
        ...
        Ok(rows.into_iter().map(|row| openleash_core::models::ApprovalRequest {
            ...
            scope: match row.7.as_str() {
                "Once" => openleash_core::models::ApprovalScope::Once,
                "Task" => openleash_core::models::ApprovalScope::Task,
                "Permanent" => openleash_core::models::ApprovalScope::Permanent,
                _ => openleash_core::models::ApprovalScope::Once,
            },
            created_at: row.8,
            expires_at: row.9,
        }).collect())
    ...
    pub async fn get_approval_request(&self, id: &Uuid) -> Result<openleash_core::models::ApprovalRequest> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| OpenLeashError::NotFound(format!("Approval request not found: {}", e)))?;

        Ok(openleash_core::models::ApprovalRequest {
            id: Uuid::parse_str(&row.0).unwrap(),
            request_id: Uuid::parse_str(&row.1).unwrap(),
            task_id: row.2.map(|u| Uuid::parse_str(&u).unwrap()),
            resource_type: match row.3.to_lowercase().as_str() {
    ...
    pub async fn get_approval_by_id(&self, id: &Uuid) -> Result<Option<openleash_core::models::ApprovalRequest>> {
        match self.get_approval_request(id).await {
            Ok(req) => Ok(Some(req)),
            Err(OpenLeashError::NotFound(_)) => Ok(None),
            Err(e) => Err(e),
        }
    }

    pub async fn update_approval_status(&self, id: &Uuid, status: &str, override_scope: Option<openleash_core::models::ApprovalScope>) -> Result<()> {
        ...
        if status == "Approved" {
            ...
            .fetch_one(&mut *tx)
            .await
            .map_err(|e| OpenLeashError::NotFound(format!("Approval request not found during update: {}", e)))?;
            ...
            if scope != openleash_core::models::ApprovalScope::Once {
                sqlx::query(
    ...
    pub async fn get_approval_by_request_id(&self, request_id: &Uuid) -> Result<Option<openleash_core::models::ApprovalRequest>> {
        ...
        if let Some(r) = row {
            Ok(Some(openleash_core::models::ApprovalRequest {
    ...
    pub async fn insert_task(&self, task: &openleash_core::models::Task) -> Result<()> {
        sqlx::query(
    ...
    pub async fn get_task(&self, id: &Uuid) -> Result<openleash_core::models::Task> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| OpenLeashError::NotFound(format!("Task not found: {}", e)))?;

        Ok(openleash_core::models::Task {
            id: Uuid::parse_str(&row.0).unwrap(),
            name: row.1,
            scope_path: row.2,
            status: match row.3.as_str() {
                "Active" => openleash_core::models::TaskStatus::Active,
                "Completed" => openleash_core::models::TaskStatus::Completed,
                "Expired" => openleash_core::models::TaskStatus::Expired,
                _ => openleash_core::models::TaskStatus::Active,
            },
            created_at: row.4,
            expires_at: row.5,
        })
    ...
    pub async fn get_expired_tasks(&self) -> Result<Vec<openleash_core::models::Task>> {
        ...
        Ok(rows.into_iter().map(|row| openleash_core::models::Task {
            id: Uuid::parse_str(&row.0).unwrap(),
            name: row.1,
            scope_path: row.2,
            status: openleash_core::models::TaskStatus::Active,
            created_at: row.4,
            expires_at: row.5,
        }).collect())
    }

    pub async fn update_task_status(&self, id: &Uuid, status: openleash_core::models::TaskStatus) -> Result<()> {
